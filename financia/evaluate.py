...
def evaluate_agent(dataset_path, model_path, initial_balance=10000, random_mode=False):
    ...
    # Print Stats
    print(
        f"{'TICKER':<10} | {'TRADES':<6} | {'WIN RATE':<9} | {'TOT. RETURN (Sum%)':<15}"
    )
    print("-" * 50)

    sorted_tickers = sorted(
        ticker_stats.items(), key=lambda x: x[1]["pnl"], reverse=True
    )
    ...
    for ticker, stats in sorted_tickers:
        ...
        if total > 0:
            ...
            print(
                f"{ticker:<10} | {total:<6} | {wr:6.1f}%   | {stats['pnl'] * 100:>.2f}%"
            )
...
def backtest_production_simulation(
    ticker,
    model_path,
    initial_balance=10000,
    random_mode=False,
    ...
    if market == "binance":
        ...
        print(
            f"    Config: 1H Signals / 1m Execution / 0m Latency (Duration: {lookback_days} days)"
        )
    else:
        ...
        latency_minutes = 15
        print(
            f"    Config: 1H Signals / 1m Execution / 15m Latency (Duration: {lookback_days} days)"
        )

    # 1. Fetch Execution Data
    print(
        f"Fetching {execution_interval} Execution Data (Last {lookback_days} days)..."
    )
    analyzer_exec = StockAnalyzer(
        ticker,
        horizon="short",
    ...
    if not live_mode:
        ...
        feature_cols = [
            ...
            if c not in ["Date", "Ticker", "Timestamp", "Close"]
        ]
        relevant_signals = df_signals[
            df_signals.index >= (sim_start - timedelta(days=1))
        ]

        # Compute normalization stats (same as training)
        feature_data = df_signals[feature_cols].values
        ...
        for idx, row in relevant_signals.iterrows():
            if random_mode:
                ...
                account_obs = np.array([0.0, 0.0, 0.5], dtype=np.float32)
        ...
        for sig_time in signal_times:
            ...
            snapshot = analyzer_signal.data[
                analyzer_signal.data.index <= sig_time
            ].copy()
            ...
            temp_analyzer.data = (
                snapshot  # This includes the "current" candle as last row
            )
            ...
            try:
                ...
                if random_mode:
                    ...
                    raw_obs = last_row[feature_cols].values.astype(np.float32)
                    obs = (raw_obs - feature_mean) / feature_std
                    obs = np.clip(obs, -3.0, 3.0)
                    account_obs = np.array([0.0, 0.0, 0.5], dtype=np.float32)
                    final_obs = np.concatenate([obs, account_obs])
                    obs_tensor = model.state_to_torch(final_obs)
                    action_tensor = model.agent.select_greedy_action(
                        obs_tensor, eval=True
                    )
                    action = action_tensor.item()
    ...
    for ts, row in df_exec.iterrows():
        ...
        while (
            next_signal_idx < len(sorted_signal_times)
            and ts >= sorted_signal_times[next_signal_idx]
        ):
        ...
        if current_signal == 1:  # BUY
            if shares == 0:
                ...
                balance = 0
                trades.append(
                    {"step": ts, "type": "buy", "price": price, "value": shares * price}
                )
        elif current_signal == 2:  # SELL
            if shares > 0:
                balance = shares * price
                trades.append(
                    {"step": ts, "type": "sell", "price": price, "value": balance}
                )
                shares = 0
    ...
    plt.plot(
        df_exec.index,
        df_exec["Close"] * (initial_balance / df_exec["Close"].iloc[0]),
        alpha=0.5,
        label="Buy & Hold",
    )
    title_suffix = " (RANDOM)" if random_mode else ""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="bist100",
        choices=["bist100", "binance"],
        help="Market to evaluate",
    )
    parser.add_argument(
        "--ticker",
        type=str,
        help="Specific ticker to evaluate (e.g., THYAO.IS or BTCUSDT)",
    )
    parser.add_argument(
        "--days",
        type=int,
        default=30,
        help="Number of days to evaluate (used with --ticker)",
    )
    parser.add_argument(
        "--model",
        type=str,
        default=None,
        help="Path to model file (auto-detected from market if not specified)",
    )
    parser.add_argument(
        "--balance",
        type=float,
        default=10000,
        help="Initial balance for the simulation",
    )
    parser.add_argument(
        "--simulation",
        action="store_true",
        help="Run Production Simulation (15m latency, 1m execution)",
    )
    parser.add_argument(
        "--random", action="store_true", help="Run with Random Agent (Benchmark)"
    )
    parser.add_argument(
        "--oracle", action="store_true", help="Show Oracle (Max Profit) Benchmark"
    )
    parser.add_argument(
        "--live",
        action="store_true",
        help="Use live/developing candle instead of closed candles",
    )
    ...
    if args.simulation:
        if not args.ticker:
            ...
            backtest_production_simulation(
                args.ticker,
                args.model,
                args.balance,
                args.random,
