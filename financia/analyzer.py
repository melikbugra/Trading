...
class StockAnalyzer:
    ...
    def _fetch_yahoo_data(self, period, interval, start, end):
        ...
        # Default defaults - enough for indicators (200 EMA needs ~200 candles + buffer)
        if period is None or interval is None:
            if self.horizon == "short":
                _period = "90d"  # ~500 hourly candles (8h/day * 90 = 720)
                _interval = "60m"
            elif self.horizon == "short-mid":
                _period = "180d"  # 1h data resampled to 4h = ~360 candles
                _interval = "1h"
