...
class StockAnalyzer:
    ...
    def _fetch_binance_data(self, period, interval, start, end):
        ...
        if period:
            ...
            if self.horizon == "short":
                days = 5  # small fetch for live analysis
            elif self.horizon == "mid":
                days = 60
            elif self.horizon == "long":
                days = 365
            else:
        ...
        try:
            ohlcv = exchange.fetch_ohlcv(
                self.ticker, use_interval, limit=limit
            )  # Fetch latest
            if not ohlcv:
                self.data = pd.DataFrame()
                return
    ...
    def _check_divergence(self, indicator, lookback=60):
        ...
        if len(p_peaks_idx) >= 2 and len(i_peaks_idx) >= 2:
            if (
                price[p_peaks_idx[-1]] > price[p_peaks_idx[-2]]
                and ind[i_peaks_idx[-1]] < ind[i_peaks_idx[-2]]
            ):
                divergence = -1
        ...
        if len(p_troughs_idx) >= 2 and len(i_troughs_idx) >= 2:
            if (
                price[p_troughs_idx[-1]] < price[p_troughs_idx[-2]]
                and ind[i_troughs_idx[-1]] > ind[i_troughs_idx[-2]]
            ):
                divergence = 1
    ...
    def _calculate_parabolic_sar(self, step=0.02, max_step=0.20):
        ...
        for i in range(1, len(close)):
            ...
            if long_trend:
                ...
                if new_sar < prev_high:
                    new_sar = prev_high
                if new_sar < current_high:
                    new_sar = current_high
    ...
    def _calculate_kama(self, n=10, pow1=2, pow2=30):
        ...
        if first_valid_idx < len(close):
            kama[first_valid_idx - 1] = close.iloc[
                first_valid_idx - 1
    ...
    def get_gator_decision(self):
        ...
        if not upper_expanding and not lower_expanding:
            phase = "SLEEPING"
        elif (upper_expanding and not lower_expanding) or (
            not upper_expanding and lower_expanding
        ):
            phase = "AWAKENING"
            ...
            phase = (
                "SATED"  # Should be covered by awakening logic actually (one reducing),
            )
    ...
    def _calculate_fisher(self, window=9):
        ...
        period_high = (
            high.rolling(window=window).max().bfill()
        )  # Use bfill to avoid NaNs at start messing up loop too much
        ...
        for i in range(1, n):
            ...
            val = (
                0.33 * 2 * ((mid_price.iloc[i] - period_low.iloc[i]) / denom - 0.5)
                + 0.67 * value[i - 1]
            )
        ...
        return pd.Series(fisher, index=self.data.index), pd.Series(
            trigger, index=self.data.index
        )
    ...
    def _calculate_supertrend(self, period=10, multiplier=3):
        ...
        return pd.Series(supertrend, index=self.data.index), pd.Series(
            trend, index=self.data.index
        )
    ...
    def calculate_final_score(self, df_decisions):
        ...
        cat_weights = {"TREND": 0.40, "MOMENTUM": 0.30, "VOLUME": 0.20, "OTHER": 0.10}
    ...
    def prepare_rl_features(self):
        ...
        df["Shadow_Down"] = (df[["Open", "Close"]].min(axis=1) - df["Low"]) / df[
            "Close"
        ]
        ...
        bb_upper, bb_mid, bb_lower = self._calculate_bollinger_bands(
            window=bb_period, num_std=bb_std
        )
        ...
        if hasattr(df.index, "hour"):
            ...
            hour = pd.Series(12, index=df.index)  # Default to noon if no time info
        ...
        if hasattr(df.index, "dayofweek"):
            ...
            dow = pd.Series(2, index=df.index)  # Default to Wednesday

        df["DayOfWeek_Sin"] = np.sin(2 * np.pi * dow / 5)  # 5 trading days
        df["DayOfWeek_Cos"] = np.cos(2 * np.pi * dow / 5)

        # --- 7. Lagged Returns (momentum memory) ---
        df["Lag_Return_1"] = df["Log_Return"].shift(1).fillna(0)
        df["Lag_Return_2"] = df["Log_Return"].shift(2).fillna(0)
        df["Lag_Return_3"] = df["Log_Return"].shift(3).fillna(0)

        # Cumulative return over last 3 bars
        df["Cum_Return_3"] = df["Log_Return"].rolling(3).sum().fillna(0)
        ...
        # Select only Feature Columns (removed correlation features, added time/lag features)
        feature_cols = [
